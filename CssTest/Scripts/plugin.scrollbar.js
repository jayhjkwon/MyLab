/************ Plugin JQuery ************* */
/* R?lis?par Creamama - Matthieu L?rat */
/* ************ juillet 2010 ************ */ 
...
(function($) {
	$.fn.scrollbar = function(params) {
		// Fusionner les param?res par d?aut et ceux de l'utilisateur
		params = $.extend( {
			taille_englobe: 'auto',			//Taille de l'espace visible - /!\ Doit ?re un nombre ou 'auto'
			taille_scrollbar: "auto",		//Taille de la scrollbar - /!\ Doit ?re un nombre ou 'auto'
			taille_bouton: 50,				//Taille du bouton - /!\ Doit ?re un nombre
			pas:75,							//Pas du scroll molette - /!\ Doit ?re un nombre ou 'auto'
			molette: true,					//D?ection du scroll molette - /!\ true ou false
			drag: true,						//Bouton de la scrollbar d?lacable ?la souris - /!\ true ou false
			debug: true,					//Afficher la console de debug - /!\ true ou false
			style: 'basic',					//Choix des styles - /!\ Par d?ault, il n'y a que le style 'basic'
			position:'droite',				//Position de la scrollBar - /!\ 'gauche' ou 'droite'
			alignement_scrollbar:'haut',	//Alignement de la scrollBar. Utilis?uniquement si elle ?une taille inf?ieur ?celle de taille_englobe
			orientation: 'vertical',		//Orientation du contenu, 'vertical' ou 'horizontal'
			marge_scroll_contenu: 15,		//Marge entre la scrollBar et le contenu - /!\ Doit ?re un nombre
		...
		return this.each(function() {
			...
			//Fonction de calcul du d?lacement du bouton
			function deplacement_bouton(info_top_contenu){
				...
				//On v?ifie que ca d?orde pas en haut
				if(depl_bouton < 0){depl_bouton = 0;}
				//On v?ifie que ca d?orde pas en bas
			...
			function controle_donnee(){
				calcul_hauteur_auto();
				//La taille du contenu doit ?re sup?ieur ?celle de l'espace visible (taille_englobe)
				if(taille_contenu > params.taille_englobe){
					//La hauteur de la scroll bar est ?ale ?la hauteur de "englobe" si hauteur_srollbar="auto"
					if(params.taille_scrollbar == "auto"){params.taille_scrollbar = params.taille_englobe;}
					//La taille de la scrollbar doit ?re inf?ieur ou ?ale ?la taille de taille_englobe
			...
			//Si la hauteur du contenu est sup?ieur ?la hauteur de l'espace visible
			if(controle_donnee()){
				//Au redimensionnement de la fenetre
				//N'est concern?par cette fonction que les ??ents en 'auto'
				...
				//On construit une div autour du contenu, mais ?l'int?ieur de la div
				...
				if(params.molette){
					$$.mousewheel(function(event, delta) {
						//On r?up?e la position du contenu
						var top_contenu = $('#englobe').css('top');
						
						//On enl?e le 'px' et on le convertit en entier pour pouvoir le manipuler
						top_contenu = parseInt( top_contenu.substring(0,(top_contenu.length-2)) );
						
						//On r?up?e la position du bouton
						...
						if (delta > 0) {							
							...
							//On v?ifie que l'on n'a pas atteint le haut du contenu
							if(top_contenu > 0){top_contenu = 0}
							$('#englobe').css({'top':top_contenu+"px"});
						//Si le delta est n?atif, c'est ?dire que l'on "ram?e" la molette
						}else if (delta < 0){							
							top_contenu = top_contenu - params.pas;
							//On v?ifie que l'on n'a pas atteint le bas du contenu
							if(top_contenu < calcul_contenu_top_max()){top_contenu = calcul_contenu_top_max()}
							$('#englobe').css({'top':top_contenu+"px"});							
						}
						
						//calcul de d?lacement du bouton					
